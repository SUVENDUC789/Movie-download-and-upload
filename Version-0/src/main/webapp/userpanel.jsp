...
<body>
	...
	String sql = "SELECT permission_tokken FROM `user_auth` WHERE username='" + (String) session.getAttribute("username")
			+ "'";
	ResultSet rs = dao.fetchData(sql);

	while (rs.next()) {
		String pt = rs.getString("permission_tokken");
		if (pt.equals("1")) {
			out.println("<div class='container my-3' id='userdetails'></div>");
		}
	}
	%>
	...
	$(document).ready(function() {
		function loadData() {
			$.ajax({
				...
				type : "POST",
				beforeSend : function() {
					$("#userdetails").html("<h1 class='text-center'>Loading data ...</h1>");
				},
